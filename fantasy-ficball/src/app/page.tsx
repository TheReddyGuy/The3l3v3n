...
const storageKey = 'ficball-simulator-v1';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const savedLeague = window.localStorage.getItem(storageKey);
    if (savedLeague) {
      try {
        const parsed = JSON.parse(savedLeague) as {
          teamName: string;
          teamMotto: string;
          gamesToPlay: number;
          players: Player[];
        ...
        setTeamName(parsed.teamName || 'Skyline Sparks');
        setTeamMotto(parsed.teamMotto || 'Play Fast. Stay Loud.');
        setGamesToPlay(clamp(parsed.gamesToPlay || 6));
  ...
  }, [teamName, teamMotto, gamesToPlay, players, results, loadState]);
  ...
  const loadDemoTeam = () => {
    setTeamName('Neon Comets');
    setTeamMotto('Pressure Creates Legends.');
    setGamesToPlay(8);
    setPlayers([
      {
        id: Date.now(),
  ...
  const resetLeague = () => {
    setTeamName('Skyline Sparks');
    setTeamMotto('Play Fast. Stay Loud.');
    setGamesToPlay(6);
    setPlayers([]);
    setResults([]);
    window.localStorage.removeItem(storageKey);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-2xl border border-blue-500/30 bg-slate-900/70 p-6 shadow-2xl shadow-blue-900/40 backdrop-blur">
          ...
          <p className="mt-3 text-xs text-cyan-200">
            Access URL: <span className="font-semibold">{accessUrl || 'Loading...'}</span> · Your league autosaves in this browser.
          </p>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_1.6fr]">
          <form onSubmit={addPlayer} className="space-y-4 rounded-2xl border border-slate-700 bg-slate-900/60 p-5">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <button
                type="button"
                onClick={loadDemoTeam}
                className="rounded-lg border border-cyan-400/40 px-3 py-2 text-sm font-semibold text-cyan-200 hover:bg-cyan-500/10"
              >
                Load Demo Team
              </button>
              <button
                type="button"
                onClick={resetLeague}
                className="rounded-lg border border-rose-400/40 px-3 py-2 text-sm font-semibold text-rose-200 hover:bg-rose-500/10"
              >
                Reset League
              </button>
            </div>
